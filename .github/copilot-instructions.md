# Copilot Instructions for Localization Scanner

This file contains instructions and guidelines for working with the Localization Scanner VS Code extension codebase.
...
* ignore is optional and can contain glob patterns to exclude files (e.g., tests, mocks)
...
* Clicking a key navigates to usage location
* No false positives
* Scans complete in reasonable time
* Errors surfaced to user clearly
...
2. **Maintain module separation** - Keep core logic separate from VS Code API layer
3. **Write pure functions** - Avoid side effects in utility functions
4. **Use explicit types** - Avoid `any` unless absolutely necessary
5. **Keep functions small** - Each function should do one thing well
6. **Early returns** - Prefer guard clauses and early returns over nested conditionals
7. **Test configuration** - Validate scan.json before processing
8. **Handle errors gracefully** - Show meaningful error messages to users
9. **Performance first** - Use async operations and efficient data structures
10. **No over-engineering** - Implement only what's needed, avoid premature abstraction